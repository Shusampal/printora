...
class SubCategory(BaseModel):
    category = models.ForeignKey(
        Category, on_delete=models.SET_NULL, null=True, blank=True, related_name="sub_category")
    category_name = models.CharField(max_length=100)
    slug = models.SlugField(unique=True, null=True, blank=True)
    is_publish = models.BooleanField(default=True)
    category_image = models.ImageField(upload_to='categories')
...
class BestSaler(BaseModel):
    product = models.ForeignKey(
        Product, on_delete=models.CASCADE, related_name='best_sale_products')
...
class HotSaler(BaseModel):
    product = models.ForeignKey(
        Product, on_delete=models.CASCADE, related_name='hot_sale_products')
...
class Onsale(BaseModel):
    product = models.ForeignKey(
        Product, on_delete=models.CASCADE, related_name='on_sale_products')
...
class DealOfTheDay(BaseModel):
    product = models.ForeignKey(
        Product, on_delete=models.CASCADE, related_name='dealoftheday')
    sale_discount_price = models.FloatField()
