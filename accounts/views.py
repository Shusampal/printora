...
def genshiptoken(request):
    ...
    response = requests.post(rurl, data=json.dumps(
        data, cls=DjangoJSONEncoder), headers=headers)
    print(response.json())
...
def userlogin(request):
    if request.method == 'POST':
        ...
        user_obj = authenticate(phone_number=phone_number, password=password)
        if user_obj:
            login(request, user_obj)
            merge_session_data_for_user(request, user_obj)
...
def shipping(invoice_nos):
    ...
    headers = {"Content-Type": "application/json",
               "Authorization": f"Bearer {shiptoken.token}"}
    ...
    for cart_item in carts.orderplaced.all():
        ...
        i = i+1
        response = requests.post(shipping_url, data=json.dumps(
            data, cls=DjangoJSONEncoder), headers=headers)
        print(response.text)
...
def register(request):
    if request.method == 'POST':
        ...
        user_obj = User.objects.create(
            full_name=full_name, phone_number=mobile, password=password)
...
def checkout(request):
    if request.method == "POST":
        ...
        totalamount = get_cart_total(request)
        if not product:
        ...
        data = {
            ...
            "redirectUrl": "https://printora.in/accounts/paymentresponse",
            "redirectMode": "POST",
            "callbackUrl": "https://printora.in/accounts/paymentcallback",
            "mobileNumber": phone,
            "paymentInstrument": {
                "type": "PAY_PAGE"
            }
        ...
        string = base64.b64encode(
            encoded_dict) + ("/pg/v1/pay").encode('utf-8') + (config('SALT_KEY')).encode('utf-8')
...
def check_status(merchantTransactionIds):
    ...
    if mydata['code'] == "PAYMENT_SUCCESS":
        ...
        try:
            cart = OrderPlaced.objects.get(
                merchantTransactionId=mydata['data']['merchantTransactionId'], is_paid=False)
        ...
        if cart.user_id:
            CartItem.objects.filter(user=cart.user).delete()
...
def paymentcallback(request):
    ...
    if mydata['code'] == "PAYMENT_SUCCESS":
        ...
        try:
            cart = OrderPlaced.objects.get(
                merchantTransactionId=merchantTransactionIds, is_paid=False)
        except Cart.DoesNotExist:
            pass
        total_cart = OrderPlaced.objects.filter(is_paid=True).count()
        # if cart.coupon:
...
def dashboard(request):
    ...
    pending_count = OrderPlaced.objects.filter(
        status='Pending', is_paid=True).count()
    packed_count = OrderPlaced.objects.filter(
        status='Packed', is_paid=True).count()
    onway_count = OrderPlaced.objects.filter(
        status='On The Way', is_paid=True).count()
    delivered_count = OrderPlaced.objects.filter(
        status='Delivered', is_paid=True).count()
...
def managepage(request):
    ...
    context = {'product': product, 'bestsaler': bestsaler,
               'hotsaler': hotsaler, 'dealoftheday': dealoftheday}
...
def on_the_way(request):
    if request.method == 'POST':
        ...
        messages.success(request, "Status Change Successfully")
    if request.user.is_staff == True:
        on_the_way = OrderPlaced.objects.filter(
            status='On The Way', is_paid=True)
...
def delivered(request):
    if request.method == 'POST':
        ...
        messages.success(request, "Status Change Successfully")
    if request.user.is_staff == True:
        delivered = OrderPlaced.objects.filter(
